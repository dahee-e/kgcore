...
def load_set_from_file(file_path):
    with open(file_path, 'r') as file:
        ...
        string_set = set(map(int, integers))
...
file_path = 'D:/Data/kg_core/case/network.hyp'
node_set = load_set_from_file('D:/Data/kg_core/case/30_15_core.dat')
print(node_set)
subhypergraph = get_induced_subhypergraph(hypergraph, node_set)

print("Nodes:", len(subhypergraph.nodes))
print("Hyperedges:", len([subhypergraph.nodes[node]['hyperedges'] for node in subhypergraph.nodes]))

u = 1
sum = 0
for p in [subhypergraph.nodes[node]['hyperedges'] for node in subhypergraph.nodes]:
    print (u, len(p), p)
    u = u+1
    sum += len(p)
print(sum / len(subhypergraph.nodes))

num_hyperedges = subhypergraph.number_of_edges()
print("Number of Hyperedges:", num_hyperedges)

neighbor_occurrence_map = construct_neighbor_occurrence_map(subhypergraph, 15)
for node, neighbors in neighbor_occurrence_map.items():
    print(node, len(neighbors), neighbors)
